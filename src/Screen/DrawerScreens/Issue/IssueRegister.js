...
import { Text, View, StyleSheet, FlatList, TouchableOpacity, Platform, Pressable, BackHandler, Switch, ScrollView } from 'react-native';
...
const IssueRegister = (props) => {
    ...
    function editEntry(item) {
        if (entryType == 1) {
            props.navigation.push('IssueEntry', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('ReturnEntry', { RegisterId: item.RegisterId });
        }
    }
    function shareEntry(item) {
        if (entryType == 1) {
            props.navigation.push('IssueTemplate', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('ReturnTemplate', { RegisterId: item.RegisterId });
        }
    }
    return (
        <>
                ...
                <View>
                    <View style={switchValue == true ? '' : styles.hide}>
                                            ...
                                            <View style={{ borderWidth: 1, borderColor: COLORS.brdleftblue, width: "100%", height: 50, paddingBottom: 5, paddingTop:10, paddingLeft:10 }} >
                                                {
                                                    Platform.OS == 'android' ?
                                                        ...
                                                        >
                                                            <Picker.Item label="Issue" value="1" />
                                                            <Picker.Item label="Return" value="2" />
                                                        </Picker>
                                                        :
                                                        <RNPickerSelect
                                                            selectedValue={entryType}
                        ...
                        <View style={{marginTop:10}}>
                                <Text style={[{width:'100%'},{ color: FONTCOLORS.primary, textAlign: 'left', fontWeight: 'bold', paddingLeft: 5, }]}>Month</Text>
                                        ...
                                        renderItem={({ item, index }) => {
                                            return <View  ><TouchableOpacity style={styles.touch}>
                                                ...
                                                <View style={styles.Flatlistview} >
                                                    ...
                                                    <View style={styles.col10}>
                                                        <View style={rview == 0 ? styles.hide : styles.col10}>
