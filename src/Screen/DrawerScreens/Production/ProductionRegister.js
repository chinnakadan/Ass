...
import { Text, View, StyleSheet, FlatList, TouchableOpacity, Pressable, BackHandler, Switch, ScrollView, Platform } from 'react-native';
...
const ProductionRegister = (props) => {
    ...
    const [rdedit, setrdEdit] = useState(0);
    const [rdview, setrdView] = useState(0);
    const [rredit, setrrEdit] = useState(0);
    const [rrview, setrrView] = useState(0);
    ...
    const retrieveData = async () => {
        try {
            ...
            setrdEdit(datas.dedit);
            setrdView(datas.dview);
            setrrEdit(datas.redit);
            setrrView(datas.rview);
            if (entryType == 1) {
                setrEdit(datas.dedit);
                setrView(datas.dview);
            } else {
                setrEdit(datas.redit);
                setrView(datas.rview);
            }
    ...
    function editEntry(item) {
        if (entryType == 2) {
            props.navigation.push('PReceiptEntry', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('PDispatchEntry', { RegisterId: item.RegisterId });
        }
    }
    function shareEntry(item) {
        if (entryType == 2) {
            props.navigation.push('PReceiptTemplate', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('DispatchTemplate', { RegisterId: item.RegisterId });
        }
    }
    return (
        <>
            ...
            <View style={styles.container}>
                <View style={[styles.Flatlistview1, { height: 40 , }]}>
                    <View style={styles.col60}>
                    </View>
                    <View style={[styles.col15]}>
                        <Text style={{ textAlign: 'right', color: FONTCOLORS.primary, fontWeight: 'bold', marginTop: 7,marginLeft:20 }}>
                            {switchValue ? 'Filter on' : 'Filter off'}
                        </Text>
                    </View>
                    <View style={[styles.col20, { marginRight: 15, marginLeft:5}]}>
                        <Switch
                            style={{ marginTop: 0 }}
                ...
                <View>
                    <View style={switchValue == true ? '' : styles.hide}>
                        <View style={styles.row1}>
                            <View style={[styles.Flatlistview2, { height: 70, marginTop: 20 }]}>
                        ...
                        <View style={[{ width: '100%', marginTop:10}]}>
                            ...
                            <View style={[styles.Flatlistview2,{ height: 50, marginTop: 0 }]}>
                                {/* <View style={styles.col66}> */}
                                    <View style={{ width: '97%', marginTop: 5, marginLeft:10}}>
                                        <View style={styles.pickerView}>
                                            <View style={{ borderWidth: 1, borderColor: COLORS.brdleftblue, width: "100%", height: 50, paddingBottom: 5,paddingTop:10, paddingLeft:10 }} >
                                                {Platform.OS == 'android' ?
                                                    <Picker
                                                    ...
                                                    >
                                                        ...
                                                        <Picker.Item label="Receipt" value="2" />
                                                    </Picker>
                                                    : <RNPickerSelect
                                                        selectedValue={entryType}
                                                        onValueChange={(itemValue, itemIndex) => changeentryType(itemValue)}
                            ...
                            <View style={[{ width: '100%', marginTop:10, marginLeft:10}]}>
                                <Text style={[{width:'100%', marginTop:10}, { color: FONTCOLORS.primary, textAlign: 'left', fontWeight: 'bold', paddingLeft: 10, }]}>Month</Text>
                                            ...
                                            return <View  ><TouchableOpacity style={styles.touch}>
                                                ...
                                                <View style={styles.Flatlistview} >
                                                    <View style={styles.col3}>
                                                        <Text style={styles.itemtittle} >Plant CostCentre Name</Text>
                                                    </View>
                                                ...
                                                <View style={styles.Flatlistview} >
                                                    <View style={styles.col3}>
                                                        <Text style={styles.itemtittle} >Dispatch To</Text>
                                                    </View>
                                                    <Text style={styles.colon}>: </Text>
                                                    <View style={styles.col7}>
                                                        <Text style={styles.assetname} >
                                                            {item.ToCostCentreName.slice(0, 15)}</Text>
                                                    </View>
                                                </View>
                                                <View style={styles.Flatlistview} >
                                                    <View style={styles.col3}>
                                                        <Text style={styles.itemtittle} >Product Name</Text>
                                                    </View>
                                                    <Text style={styles.colon}>: </Text>
                                                    <View style={styles.col7}>
                                                        <Text style={styles.assetname} >
                                                            {item.ProductName.slice(0, 15)}</Text>
                                                    </View>
                                                </View>
                                                <View style={styles.Flatlistview} >
                                                    <View style={styles.col3}>
                                                    ...
                                                    <View style={styles.col10}>
                                                        <View style={rview == 0 ? styles.hide : ''}>
                                                            <TouchableOpacity onPress={() => shareEntry(item)}>
                                                                <View style={{ flexDirection: 'row', paddingHorizontal: 4, paddingVertical: 5, borderRadius: 5 }}>
                                                                    <View>
                                                                        <Icon2 name="eye" type="ionicon" style={{ color: FONTCOLORS.primary }} size={20} />
                                                                    </View>
                                                                </View>
                                                            </TouchableOpacity>
                                                        </View>
