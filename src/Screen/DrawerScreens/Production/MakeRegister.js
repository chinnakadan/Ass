...
const MakeRegister = (props) => {
    ...
    function editEntry(item) {
        if (entryType == 1) {
            props.navigation.push('MakeEntry', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('MakeTransfer', { RegisterId: item.RegisterId });
        }
    }
    function shareEntry(item) {
        if (entryType == 1) {
            props.navigation.push('MakeTemplate', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('MakeTransferTemplate', { RegisterId: item.RegisterId });
        }
    }
    return (
        <>
                    ...
                    <View style={switchValue == true ? '' : styles.hide}>
                        ...
                        <View style={[styles.row1, { width: '100%', }]}>
                                        ...
                                        <View style={styles.pickerView}>
                                            <View style={{ borderWidth: 1, borderColor: COLORS.brdleftblue, width: "100%", height: 50, paddingBottom: 5,paddingTop:10, paddingLeft:10 }} >
                                                {Platform.OS == 'android' ?
                                                    ...
                                                    >
                                                        <Picker.Item label="Make" value="1" />
                                                        <Picker.Item label="Make and Transfer" value="2" />
                                                    </Picker>
                                                    : <RNPickerSelect
                                                        selectedValue={entryType}
                                                        onValueChange={(itemValue, itemIndex) => changeentryType(itemValue)}
                                                        items={[
                                                            { label: 'Make', value: '1' },
                                                            { label: 'Make and Transfer', value: '2' }
                                                        ]}></RNPickerSelect>
                                                }
                                            </View>
                                        </View>
                            ...
                            <View style={{width:'98%' , marginTop:10}}>
                                <Text style={[{width:'100%'} ,{ color: FONTCOLORS.primary, textAlign: 'left', fontWeight: 'bold', paddingBottom: 5,paddingTop:10, paddingLeft:10}]}>Month</Text>
                                        ...
                                        renderItem={({ item, index }) => {
                                            return <View  ><TouchableOpacity style={styles.touch}>
                                                ...
                                                <View style={styles.Flatlistview} >
                                                    ...
                                                    <View style={styles.col7}>
                                                        <Text style={styles.assetname} >
                                                            {item.PlantName.slice(0, 15)}</Text>
                                                    </View>
                                                </View>
                                                <View style={styles.Flatlistview} >
                                                    <View style={styles.col3}>
