...
const InwardRegister = (props) => {
    ...
    const [riedit, setriEdit] = useState(0);
    const [riview, setriView] = useState(0);
    const [roedit, setroEdit] = useState(0);
    const [roview, setroView] = useState(0);
    ...
    const retrieveData = async () => {
        try {
            ...
            setriEdit(datas.iedit);
            setriView(datas.iview);
            setroEdit(datas.oedit);
            setroView(datas.oview);
            if (entryType == 1) {
                setrEdit(datas.iedit);
                setrView(datas.iview);
            } else {
                setrEdit(datas.oedit);
                setrView(datas.oview);
            }
    ...
    function editEntry(item) {
        if (entryType == 2) {
            props.navigation.push('OutwardEntry', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('InwardEntry', { RegisterId: item.RegisterId });
        }
    }
    function shareEntry(item) {
        if (entryType == 2) {
            props.navigation.push('OutwardTemplate', { RegisterId: item.RegisterId });
        } else {
            props.navigation.push('InwardTemplate', { RegisterId: item.RegisterId });
        }
    }
    return (
        <>
                                        ...
                                        <View style={styles.pickerView}>
                                            <View style={{ borderWidth: 1, borderColor: COLORS.brdleftblue, width: "100%", height: 50, paddingBottom: 5, paddingTop:10, paddingLeft:10}} >
                                                {Platform.OS == 'android' ?
                                                    ...
                                                    >
                                                        <Picker.Item label="Inward" value="1" />
                                                        <Picker.Item label="Outward" value="2" />
                                                    </Picker>
                                                    : <RNPickerSelect
                                                        selectedValue={entryType}
                                                        onValueChange={(itemValue, itemIndex) => changeentryType(itemValue)}
                                  ...
                                  <FlatList style={{ width: '100%', marginBottom: 250 }}
                                            ...
                                            renderItem={({ item, index }) => {
                                                return <View  ><TouchableOpacity style={styles.touch}>
                                                    ...
                                                    <View style={entryType == 2 ? styles.hide : styles.Flatlistview} >
                                                        <View style={styles.col3}>
                                                            <Text style={styles.itemtittle} >Vendor Name</Text>
                                                        </View>
                                                        <Text style={styles.colon}>: </Text>
                                                        <View style={styles.col7}>
                                                            <Text style={styles.assetname} >
                                                                {item.VendorName.slice(0, 15)}</Text>
                                                        </View>
                                                    </View>
                                                    <View style={styles.Flatlistview} >
                                                        <View style={styles.col3}>
