...
const OutwardEntry = (props) => {
    ...
    function removeAssetData() {
        if (issueTrans.length > 0) {
            ...
            issueTrans.map((userData) => {
                let iAssetId = userData.InwardTransId;
    ...
    function deleteAssetRow(argId) {
        ...
        let index = tmptrans.findIndex(el => el.InwardTransId === argId);
        if (index !== -1) {
            ...
            {/* <ScrollView> */}
                <View style={{ marginBottom: 50 }}>
                    <FlatList style={[styles.flatlist, { marginTop: 10 }]}
                        data={issueTrans}
                        keyExtractor={item => item.InwardTransId}
                        renderItem={({ item, index }) => {
                            return <View  ><TouchableOpacity style={styles.touch}>
                                <View style={styles.Flatlistview}>
                                    <View style={styles.col7}>
                                        <Text style={styles.itemTitle2}> {item.RefNo.slice(0, 15)}   <Text style={styles.unit}>({item.RefDate})</Text> </Text>
                                    </View>
                                    <View style={approve == 'Y' ? styles.hide : styles.deleteicon}>
                                        <TouchableOpacity onPress={() => deleteRow(item.InwardTransId)}>
                                ...
                                <View style={styles.Flatlistview}>
                                    ...
                                    <View style={styles.col7}>
                                        <Text style={styles.assetname}>  {item.VendorName.slice(0, 15)}</Text>
                                    </View>
                                </View>
                                <View style={styles.Flatlistview}>
                                    <View style={styles.col3}>
                                ...
                                <View style={styles.Flatlistview}>
                                    ...
                                    <View style={styles.col3}>
                                        <TextInput
                                            ...
                                            maxLength={10}
                                            onChangeText={text => checkQty(text, item.InwardTransId)}
