...
const AssetGroupStock = (props) => {
    ...
    const actionOnRow = async (item) => {
        setCCName(item.Name);
        setModalVisible(false);
        var iResourceId = item.Id;
        setCCId(iResourceId);
        loadData(iResourceId, repType);
        setLoading(false);             
    }
    ...
    const loadData = async (argResId, argRepType) => {
        setLoading(true);
        if (argResId > 0) setselectvisible(true);
            ...
            <View style={styles.container}>
                ...
                <View style={switchValue == true ? '' : styles.hide}>
                    <View style={styles.mtxttop}>
                        <Text style={styles.assetname}>Asset Group</Text>
                        <View style={styles.centeredViewrow}>
                            ...
                            >
                                <Text style={styles.textStyle}>{CCName ? CCName : 'Select Asset Group'}</Text>
                            </Pressable>
                        </View>
                    </View>
                    <View style={styles.mtxttop}>
                ...
                <ScrollView style={selectvisible == true ? '' : styles.hide}>
                    <FlatList style={styles.flatlist}
                        ...
                        renderItem={({ item, index }) => {
                            return <View  ><TouchableOpacity style={styles.touch} onPress={() => { props.navigation.navigate('AssetCCGStock', { AssetId: item.AssetId, AssetName: item.AssetName, RepType: repType }) }}>
...
export default AssetGroupStock;
